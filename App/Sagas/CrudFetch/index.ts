...
// app component
import { restClient, HttpError, GET_BEVERAGE } from '../../Services';
import { CrudAction, fetchStart, fetchEnd, fetchError, fetchCancel, crudGetUserInfoFinish } from '../../Redux';
...
function* handleFetch(action: CrudAction) {
    ...
    const { payloadRequest, metaRequest: { fetch: fetchMeta } } = action;
    ...
    try {
        ...
        response = yield call(restClient, restType, payloadRequest);
        ...
        if (restType === GET_BEVERAGE) {
            yield put(crudGetUserInfoFinish(response));
        }
        ...
        yield put(fetchError(httpError.status));
    } finally {
